...
(function () {
  ...
  /* ─────────────────────────────────────────────────
     PRODUCTS
  ───────────────────────────────────────────────── */
  // ستون‌هایی که واقعاً در کارت/گریدِ محصولات (صفحه‌ی اصلی، shop.html) استفاده
  // می‌شن. قبلاً select('*') کل محصول رو می‌کشید — شامل story_fa/story_en،
  // specs، care_fa/care_en، install_fa/install_en، companion و tech_img که
  // فقط توی صفحه‌ی جزئیاتِ یک محصول (getProduct) لازمن، نه توی لیست. این
  // فیلدهای سنگین رو این‌جا حذف کردیم تا حجمِ هر بار لودشدنِ لیستِ محصولات
  // (که روی هر بازدیدکننده اجرا می‌شه) به‌شدت کم بشه.
  const LIST_COLUMNS =
    'id, var_group, var_order, variant_label, category, sub, name_fa, name_en, ' +
    'price, price_num, image, image2, img_pos, imgs, alts, badge, is_featured, ' +
    'is_active, desc_fa, desc_en';

  async function getProducts({ category, sub, featured } = {}) {
    let query = db.from('products').select(LIST_COLUMNS).eq('is_active', true);
  ...
  // getProduct(id) عمداً select('*') می‌مونه — صفحه‌ی جزئیاتِ محصول واقعاً به
  // همه‌ی فیلدها (توضیحات کامل، مشخصات فنی، نگه‌داری، نصب) نیاز داره.
